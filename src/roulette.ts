...
export class Roulette extends EventTarget {
  ...
  public start() {
    ...
    const startAndBoost = () => {
      ...
      // 관리자 모드: 지정된 마블에게 부스트
      if (options.adminMode && options.riggedWinner) {
        const riggedMarble = this._marbles.find(m => m.name === options.riggedWinner);
        if (riggedMarble) {
          this.physics.boostMarble(riggedMarble.id);
        }
      }
    };

    if (this._autoRecording) {
      this._recorder.start().then(startAndBoost);
    } else {
      startAndBoost();
    }
  }
  ...
  public setMarbles(names: string[]) {
    ...
    // 관리자 모드: riggedWinner가 있으면 해당 마블에게 order 0 부여
    let riggedOrder: number | null = null;
    if (options.adminMode && options.riggedWinner) {
      riggedOrder = 0;
      const zeroIndex = orders.indexOf(0);
      if (zeroIndex > -1) {
        orders.splice(zeroIndex, 1);
      }
    }
    ...
    members.forEach((member) => {
      if (member) {
        for (let j = 0; j < member.count; j++) {
          let order: number;
          if (riggedOrder !== null && member.name === options.riggedWinner && j === 0) {
            order = riggedOrder;
            riggedOrder = null;
          } else {
            order = orders.pop() || 0;
